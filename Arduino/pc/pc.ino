...
void loop(void) {
	...
	if (Serial.available() > 0) 
	{
		...
		if (inChar == '\n') 
		{
			if(inputString.indexOf("setWaterLevelLow") != -1)
			{
				waterLevelLow = getSerialValue(inputString);
			}
			else if(inputString.indexOf("setWaterLevelHigh") != -1) 
			{
	...
	if((currentMillis - previousMillis) > interval) 
	{
		...
		waterLevel +=50;
		dtostrf(waterLevel,5,1,levelStr);
		...
		if((waterLevel < waterLevelLow) && (waterState == STATE_WATER_OFF))
		{
			digitalWrite(WATER_CONTROL_PIN, HIGH);
			waterState = STATE_WATER_ON;
		}
		else if((waterLevel > waterLevelHigh) && (waterState == STATE_WATER_ON))
		{
			digitalWrite(WATER_CONTROL_PIN, LOW);
			waterState = STATE_WATER_OFF;
		}
