...
run:
	erl -pa ebin/ deps/*/ebin/ -config etc/pc.config -config etc/app.config -config etc/sync.config -config etc/simple_bridge.config -eval "application:start(sync)" -eval "application:start(sasl)" -eval "application:start(pc)" -eval "nitrogen_sup:start_link()" -sname boan -setcookie hej -detached
